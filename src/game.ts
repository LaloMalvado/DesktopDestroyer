...
ctx.stroke();
ctx.globalAlpha=1}ctx.restore()}const h={init(){},resetForRun(){},hitAt(x,y){Effects.spawnHit(x,y,{dpr});
playHammerEffect();
...
Bugs.hitAt(x,y,'hammer');}};function impactHammer(x,y){h.hitAt(x,y);}function burnAt(x,y,dt){Effects.spawnFlame(x,y,{dpr});
...
Bugs.hitAt(x,y,'gun');}const f={init(){},resetForRun(){stopFlameAudio();},hitAt(x,y,dt){burnAt(x,y,dt||0);}};
const i={init(){},resetForRun(){},hitAt(x,y,dt){sprayAt(x,y,dt||0);}};
const g={init(){},resetForRun(){shootCooldown=0;},hitAt(x,y){shootAt(x,y);}};
const tools={h,f,i,g};
...
function setTool(t){const prev=currentToolName();
const nextKey=TOOL_KEY_BY_NAME[t]||'h';
const nextName=TOOL_NAME_BY_KEY[nextKey]||'hammer';
toolKey=nextKey;
if(prev==='flame'&&nextName!=='flame')stopFlameAudio();
if(prev==='spray'&&nextName!=='spray')stopSprayAudio();
...
if(!runStart)return;
ensureAudio();
const activeTool=currentToolName();
if(activeTool==='flame')startFlameAudio();
if(activeTool==='spray')startSprayAudio();
if(activeTool==='hammer')currentToolObj().hitAt(pointer.x,pointer.y);
else if(activeTool==='gun')currentToolObj().hitAt(pointer.x,pointer.y)});
canvas.addEventListener('pointerup',()=>{pointer.down=false;
const activeTool=currentToolName();
if(activeTool==='flame')stopFlameAudio();
if(activeTool==='spray')stopSprayAudio()});
...
return}const activeTool=currentToolName();
if(activeTool==='flame'&&pointer.down)currentToolObj().hitAt(pointer.x,pointer.y,dt);
if(activeTool==='spray'&&pointer.down)currentToolObj().hitAt(pointer.x,pointer.y,dt);
if(activeTool==='gun'&&pointer.down){shootCooldown-=dt;
if(shootCooldown<=0){currentToolObj().hitAt(pointer.x,pointer.y);
shootCooldown=.045;}}
Bugs.update(dt);
ctx.drawImage(damageCanvas,0,0);
ctx.drawImage(voidCanvas,0,0);
