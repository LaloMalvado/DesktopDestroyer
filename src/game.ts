...
let W=0,H=0,last=0,fps=0,shakeT=0,tool='hammer';
...
Bugs.clampCap();
updateHUD()}function addFlameParticles(x,y){for(let i=0;
i<18;
i++){const ang=Math.random()*Math.PI*2,spd=60+Math.random()*160;
particles.push({x,y,vx:Math.cos(ang)*spd,vy:Math.sin(ang)*spd-(40+Math.random()*80),life:.22+Math.random()*.45,age:0,size:(2+Math.random()*2.4)*dpr,type:'flame'})}for(let i=0;
i<10;
i++){const ang=Math.random()*Math.PI*.6-Math.PI/3,spd=30+Math.random()*60,tones=['red','orange','yellow'],tone=tones[(Math.random()*tones.length)|0];
particles.push({x,y,vx:Math.cos(ang)*spd,vy:Math.sin(ang)*spd-30,life:.45+Math.random()*.8,age:0,size:(1.6+Math.random()*2.2)*dpr,type:'ember',tone})}for(let i=0;
i<6;
i++)addSpark(x,y)}function addSpark(x,y){for(let i=0;
i<8;
i++){const ang=Math.random()*Math.PI*2,spd=120+Math.random()*120;
particles.push({x,y,vx:Math.cos(ang)*spd,vy:Math.sin(ang)*spd,life:.08+Math.random()*.12,age:0,size:(1.5+Math.random()*1.8)*dpr,type:'spark'})}}function eatAt(x,y,r){voidCtx.save();
...
ctx.stroke();
ctx.globalAlpha=1}ctx.restore()}function impactHammer(x,y){playHammer();
...
damageCtx.stroke()}shakeT=120;
Bugs.hitAt(x,y,'hammer');}function burnAt(x,y,dt){addFlameParticles(x,y);
Bugs.hitAt(x,y,'flame',{dt});}function sprayAt(x,y,dt){for(let i=0;
i<20;
i++)particles.push({x,y,vx:(Math.random()*2-1)*60,vy:(Math.random()*2-1)*60,life:.3+Math.random()*.6,age:0,size:(2+Math.random()*2.5)*dpr,type:'toxic'});
...
damageCtx.restore();
addSpark(x,y);
shakeT=Math.max(shakeT,60);
Bugs.hitAt(x,y,'gun');}function setTool(t){const prev=tool;
tool=t;
if(prev==='flame'&&t!=='flame')stopFlameAudio();
if(prev==='spray'&&t!=='spray')stopSprayAudio();
document.querySelectorAll('.btn[data-tool]').forEach(b=>b.classList.toggle('active',b.dataset.tool===t));
updateHUD()}canvas.addEventListener('pointerdown',e=>{pointer.down=true;
updatePointer(e);
if(!runStart)return;
ensureAudio();
if(tool==='flame')startFlameAudio();
if(tool==='spray')startSprayAudio();
if(tool==='hammer')impactHammer(pointer.x,pointer.y);
else if(tool==='gun')shootAt(pointer.x,pointer.y)});
canvas.addEventListener('pointerup',()=>{pointer.down=false;
if(tool==='flame')stopFlameAudio();
if(tool==='spray')stopSprayAudio()});
...
else if(k==='m'){const guide=document.getElementById('guidePanel'),l=document.getElementById('legend');
[guide,l].forEach(el=>{if(!el)return;
el.style.display=el.style.display==='none'?'':'none'})}else if(k==='v'){const hud=document.getElementById('hud'),guide=document.getElementById('guidePanel'),l=document.getElementById('legend');
[hud,guide,l].forEach(el=>{if(!el)return;
...
resize();
Bugs.init({dpr,getBounds:()=>({width:W,height:H}),getEatRate:()=>EAT_RATE,onBugEat:(x,y,r)=>eatAt(x,y,r),onBugKilled:handleBugKilled,onBugRevived:handleBugRevived,onBravioSpawn:(x,y)=>addFlameParticles(x,y),onTorazoRevive:emitReviveEmbers});
...
const toolName={hammer:dict.hammer,flame:dict.flame,spray:dict.spray,gun:dict.gun}[tool];
$('#statTool').textContent=(I18N[LANG].tool||'Herramienta')+': '+toolName;
...
function emitReviveEmbers(b){for(let i=0;i<12;i++)particles.push({x:b.x,y:b.y,vx:(Math.random()*2-1)*40,vy:(Math.random()*2-1)*40,life:.35+Math.random()*.4,age:0,size:(1.8+Math.random()*2.2)*dpr,type:'ember',tone:'yellow'})}
function doReset(){
LOST=false;
WIN_SHOWN=false;
...
if(shakeT>0){const s=shakeT/120*3*dpr;
ctx.translate((Math.random()*2-1)*s,(Math.random()*2-1)*s);
shakeT-=dt*1000}paintBackground();
const bugList=Bugs.getAll();
if(!runStart){ctx.drawImage(damageCanvas,0,0);
...
return}if(tool==='flame'&&pointer.down)burnAt(pointer.x,pointer.y,dt);
if(tool==='spray'&&pointer.down)sprayAt(pointer.x,pointer.y,dt);
if(tool==='gun'&&pointer.down){shootCooldown-=dt;
if(shootCooldown<=0){shootAt(pointer.x,pointer.y);
shootCooldown=.045;}}
Bugs.update(dt);
...
continue}p.x+=p.vx*dt;
p.y+=p.vy*dt;
const t=p.age/p.life,r=p.size*(1-.7*t);
...
return`rgba(255,220,60,${a})`}function assert(c,m){dbg((c?'✅ ':'❌ ')+m)}function runSelfTests(){try{const before=tool;
setTool('flame');
assert(tool==='flame','setTool ok');
setTool(before);
...
addFlameParticles(100,100);
assert(particles.length>p0,'particles added');
window.dispatchEvent(new KeyboardEvent('keydown',{key:'f'}));
assert(tool==='flame','key f');
window.dispatchEvent(new KeyboardEvent('keydown',{key:'h'}));
assert(tool==='hammer','key h');
...
if(!validateEmail(p.email)){dbg('saveBest: sin email @zetabe.com;
 no se envía');
